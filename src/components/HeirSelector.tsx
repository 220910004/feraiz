...
export const HeirSelector: React.FC<HeirSelectorProps> = ({
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-[22px] border p-4 text-sm shadow-sm" style={{borderColor: 'var(--accent-border)', background: 'var(--brand-lighter)', color: 'var(--brand-primary)'}}>
        <p className="font-medium">{locale.chooserTitle}</p>
        <p className="mt-2 leading-5" style={{color: 'var(--brand-secondary)', opacity: 0.85}}>
        ...
        {categories.map((category) => {
          ...
          return (
            <section
              ...
              style={{borderLeftWidth: '3px', borderLeftColor: selectedCountInCategory > 0 ? 'var(--brand-primary)' : 'var(--border-1)'}}
            >
              <div className="mb-4 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
                <div>
                  <div className="flex items-center gap-3">
                    <span className="text-2xl">{category.icon}</span>
                    <h3 className="text-lg font-semibold" style={{color: selectedCountInCategory > 0 ? 'var(--brand-primary)' : 'var(--text-1)'}}>{category.label}</h3>
                    {selectedCountInCategory > 0 ? (
                      <span className="rounded-full px-2.5 py-1 text-xs font-medium text-white" style={{background: 'var(--brand-primary)'}}>{selectedCountInCategory} {locale.selected}</span>
                    ) : (
                      <span className="rounded-full border border-stone-200 bg-stone-50 px-2.5 py-1 text-xs text-stone-500">{selectedCountInCategory} {locale.selected}</span>
                    )}
                  </div>
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {count > 0 && (
                              <span className="rounded-full px-2 py-0.5 text-[11px] text-white" style={{background: 'var(--brand-primary)'}}>
                                {locale.chosen}
                              </span>
                            )}
                            {blockInfo.blocked && (
